...
// UPDATE SATPAM: Admin diberikan hak akses penuh ke route Kasir
const ProtectedRoute = ({ allowedRole }) => {
  const user = JSON.parse(localStorage.getItem('user'));
  if (!user) return <Navigate to="/login" replace />;
  
  // 1. Jika aksesnya 'all', biarkan masuk
  // 2. Jika user adalah 'admin', biarkan masuk ke mana saja (termasuk zona kasir)
  // 3. Jika bukan keduanya, baru validasi apakah role user sesuai dengan allowedRole
  const isAllowed = 
    allowedRole === 'all' || 
    user.role === 'admin' || 
  ...
  if (!isAllowed) {
    // Jika kasir mencoba akses zona admin, lempar ke POS Kasir
      ...
      <Routes>
        ...
        <Route element={<ProtectedRoute allowedRole="admin" />}>
          <Route element={<AdminLayout />}>
            ...
            <Route path="/admin/expense" element={<AdminExpenses />} />
          </Route>
        ...
        <Route element={<ProtectedRoute allowedRole="kasir" />}>
          <Route element={<KasirLayout />}>
            ...
            <Route path="/kasir/BiayaOprasional" element={<CashierExpenses />} />
          </Route>
        </Route>
