...
export default function AdminAttendance() {
  ...
  const [deletingId, setDeletingId]     = useState(null); // track loading hapus
  ...
  /* ── Hapus status sakit / izin ── */
  const handleRemoveStatus = async (log) => {
    if (!window.confirm(`Hapus status ${log.status === 'sick' ? 'Sakit' : 'Izin'} untuk ${log.user_name}?`)) {
      return;
    }

    setDeletingId(log.id);
    try {
      // Sesuaikan endpoint sesuai backend Anda
      await api.delete(`/api/hr/attendance/${log.id}`);
      // atau: await api.patch(`/api/hr/attendance/${log.id}`, { status: null });
              ...
              <tbody>
                {isFetching ? (
                  ...
                  filtered.map((log) => {
                    ...
                    const isAbsent = log.status === 'sick' || log.status === 'leave';
                    ...
                    return (
                      <tr key={log.id}>
                        ...
                        <td>
                          ...
                          {/* Tombol Hapus — hanya untuk Sakit & Izin */}
                          {isAbsent && (
                            <div>
                              <button
                                className="btn-remove"
                                disabled={deletingId === log.id}
                                onClick={() => handleRemoveStatus(log)}
