...
const Icons = {
  ...
  Spinner: () => (
    <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" fill="none" strokeWidth="2.5" className="spin">
...
export default function Login() {
  // Satu field untuk email ATAU nama
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Kirim sebagai "identifier" supaya backend bisa cari by email atau name
      const response = await api.post('/api/auth/login', {
        identifier,   // bisa diisi email atau nama
        password
      });
          ...
          <div className="login-body">
            <form onSubmit={handleLogin}>
              ...
              <div className="f-group">
                <label className="f-label">Email atau Nama</label>
                <div className="inp-wrap">
                  <input 
                    type="text"                 /* text, bukan email */
                    required
                    value={identifier}
                    onChange={(e) => setIdentifier(e.target.value)}
                    className="f-inp"
                    placeholder="admin@kafe.com atau Admin"
            ...
            <div className="demo-section">
              ...
              <div className="demo-grid">
                {[
                  { name: 'ansor', email: 'ansor@gmail.com', password: 'password', role: 'admin' },
                  { name: 'koyan', email: 'koyan@gmail.com', password: 'password', role: 'kasir' },
                  { name: 'riri', email: 'riri@gmail.com', password: 'password', role: 'kasir' },
                  { name: 'mj', email: 'mj@gmail.com', password: 'password', role: 'kasir' },
                  { name: 'Admin Utama', email: 'mj@gmail.com', password: 'password', role: 'kasir' },
                ].map((acc) => (
                  <div 
                    key={acc.email} 
                    className="demo-card" 
                    onClick={() => { 
                      // Bisa isi dengan email ATAU nama
                      setIdentifier(acc.name);   // atau setIdentifier(acc.name);
